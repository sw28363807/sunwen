(function () {
    ...
    class Cat extends Spine {
        ...
        //初始化叹号
        initMaskAnim() {
            this.maskAnim = new Laya.Skeleton();
            this.maskAnim.load("anim/UIdonghua.sk", Laya.Handler.create(this, function() {
                this.maskAnim.y = -250;
                this.addChild(this.maskAnim);
                this.maskAnim.visible = false;
            }));
        }

        showMaskAnim(handler) {
            this.maskAnim.visible = true;
            this.playAnim("lick1");
            this.maskAnim.play("tanhao");
        ...
        popSelf() {
            this.__oldParent = this.parent;
            this.__oldX = this.x;
            this.__oldY = this.y;
            this.__oldZOrder = this.zOrder;
            this.removeSelf();
        }

        restoreSelf() {
            this.removeSelf();
            this.__oldParent.addChild(this);
            this.pos(this.__oldX, this.__oldY);
            this.zOrder = this.__oldZOrder;
            this.updateZOrder();
        }
    ...
    class BuildingData{
        ...
        constructor(bid) {
            ...
            this.hasIdleBuildingRoot = false;
    ...
    class CommonUIMgr {
        ...
        showCommonUI(sceneId, handler) {
            let completeHandler = new Laya.Handler(this, function() {
    ...
    class SceneMgr {
        ...
        loadScene(sceneId, handler) {
            ...
            if(scene != null) {
                ...
                Laya.Scene.open(sceneUrl, false, null, new Laya.Handler(this, function(scene) {
    ...
    class BuildingSystem {
        ...
        static randomVisitBuilding(sceneId, actionEvent) {
            ...
            buildingDataList.forEach(buildingData => {
                if(buildingData != null && buildingData.getLocationData()!= null &&
            ...
            let i = Utils.randomExt(arr.length);
            let ret = arr[i];
            return ret;
        }
    ...
    class CatFactory {
        ...
        createCoffeeCat(catData, parent, completeHandler) {
            let cat = new Cat(catData);
            cat.addLoadedHandler(function() {
                parent.addChild(cat);
                cat.playAnim("lick1", true);
                Utils.fadein(cat, 500, new Laya.Handler(this, function() {
                    if(completeHandler != null) {
                        completeHandler();
                    }
                }));
        ...
        createBeachCat(catData, parent, completeHandler) {
            let cat = new Cat(catData);
            cat.addLoadedHandler(function() {
                parent.addChild(cat);
                cat.playAnim("lick1", true);
    ...
    class CatData {
        ...
        randomFood() {
            let s = Utils.randomExt(this.foodWeighted);
            let w = 0;
            for(let i = 0; i < this.foodWeightedArray.length; ++i) {
                let a = this.foodWeightedArray[i];
                let d = a.w + w;
                if(w <= s && s <= d) {
                    return a.c;
                }
    ...
    class CatSystem {
        ...
        static addCatData(configId, sceneId, params) {
            ...
            if(sceneId==Constants.CoffeeSceneId) {
                EventMgr.getInstance().postEvent("SIMULATOR_SEND_CMD", [sceneId, "ADD_CAT", catData]);
            }
            return catData;
        }
        ...
        static removeCatData(catId, sceneId) {
            ...
            if(sceneId==Constants.CoffeeSceneId) {
                ...
                EventMgr.getInstance().postEvent("REMOVE_BEACH_CAT", catData);
            }
        }
        ...
        static randomFood(catData) {
            let ret = null;
            let foodType = catData.randomFood();
            let config = ConfigMgr.getInstance().getConfig("Foods");
    ...
    class SyncDataMgr {
        ...
        dumpRewards(){
            if(this._fishReward>0){
                return {"rewards": [{"id": Constants.RewardType.FISH, "count": this._fishReward}]};
            }
        }
    ...
    class PetData {
        ...
        getIntimacyGrade() {
            ...
            for(const k in this.config.Intimacy) {
                let intimacy = this.config.Intimacy[k];
                if(intimacy<v){
                    return k - 1;
                }
            }
            return this.config.Intimacy.length;
        }
    ...
    class PetReward {
        ...
        dumpAndStash() {
            ...
            if(this.intimacyIncr>0 || this.visitTimesIncr>0) {
                this.stash = {"intimacyIncr":this.intimacyIncr==0?null:this.intimacyIncr, "visitTimesIncr":this.visitTimesIncr==0?null:this.visitTimesIncr};
                return this.stash;
            }
            return null;
        }
    ...
    class PetDataMgr {
        ...
        updatePet(petId, intimacy, visitTimes, adopted) {
    ...
    class SyncSystem {
        ...
        static updateSceneInfo(handler) {
            ...
            let customData = JSON.stringify({cats: CatDataMgr.getInstance().dump(), fish: SyncDataMgr.getInstance().dumpCachedFish()});
            ...
            EventMgr.getInstance().postEvent("SERVICE_UPDATE_SCENE_INFO", [customData, rewards, petRewards, visitorRewards, Laya.Handler.create(null, dealFunc)]);
        }
        ...
        static restoreCustomData(customData) {
            ...
            EventMgr.getInstance().postEvent("CUSTOMDATA_CAT_RESTORE", [data.cats]);
        }
        ...
        static collectFish(buildingId) {
            let fish = SyncDataMgr.getInstance().collectFish(buildingId);
    ...
    class BuildingExtInfoMgr {
        ...
        getNotBusyBuildingExtInfo() {
            for(let i = 0 ; i < this.buildingExtInfoArray.length; ++i) {
                let ret = this.buildingExtInfoArray[i];
                if(ret.getIsBusy() == false) {
                    return ret;
                }
            }
            return null;
        ...
        clearInteractCat() {
            for(let i = 0; i<this.buildingExtInfoArray.length; ++i) {
                let info = this.buildingExtInfoArray[i];
                let buildingRoot = info.getBuildingRoot();
                if(info != null && buildingRoot != null) {
                    for(let i = 0; i < buildingRoot.numChildren; ++i) {
                        let child = buildingRoot.getChildAt(0);
                        if(child != null) {
                            // EventMgr.getInstance().postEvent("REMOVE_COFFEE_CAT", child.getData().getCatId());
                            CatSystem.removeCatData(child.getData().getCatId(), this.buildingData.getBelongScene());
                        }
                    }
                }
            }
        ...
        setDropFishPoint(dropPoint) {
            ...
            if(this.dropPoint != null) {
                this.dropPoint.on(Laya.Event.MOUSE_DOWN, this, function() {
                    ...
                    this.dropFish = null;
                    if(fish){
                        SyncSystem.collectFish(this.buildingData.getId());
                        fish.onClick();
                        fish.zOrder = 9999999;
    ...
    class Building$1 extends Laya.Tree {
        ...
        cleanBuilding() {
            ...
            this.getBuildingExtInfoMgr().clearInteractCat();
        }
                ...
                if(spineAnimText != null &&
                    spineAnimText.text != null &&
                     spineAnimText.text != "") {
                        ...
                        let anim = new Laya.Skeleton();
                        anim.load(animPath, Laya.Handler.create(this, function() {
    ...
    class BuildingFactory {
        ...
        createBuilding(buildingData, parent, completeHandler) {
            ...
            building.loadResource(buildingData.getSource(), function() {
                ...
                Utils.fadein(building, 1000, new Laya.Handler(this, function(){
                    if(completeHandler != null) {
    ...
    class ShowCatMenuTip {
        ...
        constructor(cat, buildingExtInfo, anim, time, handler) {
            this.cat = cat;
        ...
        initCatMenuTip() {
            ...
            let foodData = CatSystem.randomFood(this.cat.getData());
            let catTipImage = this.menuTipPrefab.getChildByName("meals_img");
            catTipImage.loadImage(foodData.Source, Laya.Handler.create(this, function() {
                catTipImage.x -= catTipImage.width/2.0*catTipImage.scaleX;
                catTipImage.y -= catTipImage.height/2.0*catTipImage.scaleY;
            }));
            this.menuTipPrefab.y = -250;
            this.menuTipPrefab.x = -10;
    ...
    class CoffeeCatTLInterpreter extends TLInterpreter{
        ...
        IntoBuilding(action, args, mode) {
            if(mode==1){
                let owner = this._cat;
                let type = args.type;
                let buildingExtInfo = args.buildingExtInfo;
                let time = args.time;

                owner.showMaskAnim(new Laya.Handler(this, function() {
                    Utils.fadeout(owner, 500, new Laya.Handler(this, function() {
                        if(buildingExtInfo != null) {
                            ...
                            if(type == 1) {
                                ...
                                let anim = new Laya.Skeleton();
                                anim.load("anim/UIdonghua.sk", Laya.Handler.create(this, function() {
                                    anim.y = -150;
                                    cat.addChild(anim);
                                    anim.visible = false;
                                    owner.catMenuTips = new ShowCatMenuTip(cat, buildingExtInfo, anim, time, null);
                                }));
                            }
                            return cat;
                        }
        ...
        CatEat(action, args, mode) {
            ...
            if(owner.catMenuTips){
                ...
                owner.catMenuTips.anim.visible = true;
                owner.catMenuTips.anim.play("chifan2", true);
            }
        }
        ...
        WaitEatTimeOut(action, args, mode) {
            ...
            if(owner.catMenuTips){
                ...
                owner.catMenuTips.anim.visible = true;
                owner.catMenuTips.menuTipPrefab.visible = false;
                owner.catMenuTips.anim.play("shengqi", true);
            }
        }
    ...
    class PetSystem {
        ...
        static dumpPetRewards() {
            return PetDataMgr.getInstance().dumpPetRewards();
        }
        ...
        static updatePets(pets) {
            Object.getOwnPropertyNames(pets).forEach(key=>{
                ...
                PetDataMgr.getInstance().updatePet(petId, intimacy, visitTimes, adopted);
            });
        }
    ...
    class CatLogicHelper {
            //获得奖励
        static getReward(eventData, valueBase) {
            let ret = null;
            let completeReward = eventData.CompleteReward;
            let probability = Utils.randomExt(100) + 1;

            for(const key in completeReward) {
                let element = completeReward[key];
                if(probability>0 && element != null && element != "") {
                    let rewardArr =  element.split(":");
                    let weight = Number(rewardArr[0]);
                    let reward = Number(rewardArr[1]);
                    if(weight >= probability ) {
                        ret = reward * valueBase;
                        probability = -1;
                        break;
                    }else{
        ...
        //寻找可以执行动作的空位
        //1 有空位 2 没有空位 3 building或者buildingRoots消失了
        static findCatMoveBlock(buildingId, sceneId) {
            let building = SceneMgr.getInstance().getScene(sceneId).getBuilding(buildingId);
            if(building == null) {
                return {t: 3, b: null};
            }
            if(building.getBuildingExtInfoMgr().getBuildingExtInfoCount() == 0) {
                return {t: 3, b: null};
            }
            let buildingExtInfo = building.getBuildingExtInfoMgr().getNotBusyBuildingExtInfo();
            if (buildingExtInfo != null) {
                return {t: 1, b: buildingExtInfo};
            }
            return {t: 2, b: null};
        }
    ...
    class CoffeeCatRole {
        ...
        start(context) {
            ...
            this._context.recordMetaEvent(this.getId(), "AddCat", this.catData, "CoffeeCatLine");
        }
        ...
        eat() {
            if(this._state=="waitEat") {
                this._state=="eating";
                this._setNextStep("outBuilding", 6000, this._eatInfo);
                this._context.recordEvent(this.getId(), "CatEat", null, "CoffeeCatLine");
            }
        }
        ...
        remove() {
            ...
            if(this._state!="leaving"){
                this._context.recordEvent(this.getId(), "RemoveCat", null, "CoffeeCatLine");
                return true;
            }
            return false;
        }

        _onActive() {//把这些SceneMgr后期替换
            let buildingExtInfoCount = SceneMgr.getInstance().getScene(Constants.CoffeeSceneId).getBuildingRootInfoCount();
            let activeCatCount = this._context._roleMgr.getActiveRoleCount();
            if(activeCatCount <= buildingExtInfoCount + 6) {
        ...
        _doLeave() {
            ...
            this._state = "leaving";
            // this._gotoTargetIndex(0, new Laya.Handler(this, function() {
            //     Utils.fadeout(this.owner, 1000, new Laya.Handler(this, function() {
            //         CatSystem.removeCatData(this.owner.getData().getCatId(), Constants.CoffeeSceneId);
            //     }));
            // }));
        }
        ...
        _remove() {
            ...
            this._context.recordEvent(this.getId(), "RemoveFade", null, "CoffeeCatLine");
        ...
        _gotoTargetIndexAndStand(targetPathIndex) {
            ...
            if(g > 97) {
                //TODO 记录 randomStand
                // CatLogicHelper.randomStand(this.owner, 3000 + Utils.randomExt(6000), new Laya.Handler(this, function() {
                //     this._gotoTargetIndex(targetPathIndex, handler);
                // }));
                this._state = "randomStand";
                this._context.recordEvent(this.getId(), "RandomStand", null, "CoffeeCatLine");
                this._setNextStep("gotoTargetIndex", 3000 + Utils.randomExt(6000), targetPathIndex);
            } else {
        ...
        _gotoTargetIndex(targetPathIndex) {
            if(this.pathIndex != targetPathIndex) {
                ...
                // TODO 记录Move 计算时间 zorder
                // CatLogicHelper.doMoveTo(this.owner, this.speed, p.x, p.y, new Laya.Handler(this, function() {
                //     if (p.i == ScenePathMgr.getInstance().getZOrderPathPoint()) {
                //         if(targetPathIndex > this.pathIndex) {
                //             this.owner.zOrder = this.upOrder;
                //         } else if (targetPathIndex < this.pathIndex) {
                //             this.owner.zOrder = this.downOrder;
                //         }
                //     }
                //     this._gotoTargetIndexAndStand(targetPathIndex, handler);
                // }));
                ...
                this._state = "moving";
                this._context.recordEvent(this.getId(), "MoveTo", {"x":p.x, "y":p.y, "time":time, "zOrder":zOrder}, "CoffeeCatLine");
        ...
        _waitEmpty(eventData) {
            ...
            this._context.recordEvent(this.getId(), "Wait", {"anim":"lick1"}, "CoffeeCatLine");
            let buildingId = this.targetBuildingData.getId();
            let info = CatLogicHelper.findCatMoveBlock(buildingId, Constants.CoffeeSceneId);
            if(info.t == 1) {
                if(info.b != null) {
                    this._setNextStep("intoBuilding", 0, [eventData, buildingId, info.b]);
                }
            } else if (info.t == 2 && info.b != null) {
        ...
        _intoBuilding(info) {
            if(info && info[0] && info[1]) {
                ...
                let buildingExtInfo = info[2];
                buildingExtInfo.setIsBusy(true);
                let type = 1;
                ...
                if(type==1){
                    ...
                    this._state = "waitEat";
                    this._eatInfo = info;
                    this._setNextStep("waitEatTimeOut", 10900, info);
                    this._context.recordEvent(this.getId(), "IntoBuilding", {"type":type, "time": 10900, "buildingExtInfo":buildingExtInfo}, "CoffeeCatLine");
                }else if(type==2){
                    //休息
                    this._state = "rest";
                    this._setNextStep("outBuilding", 6900, info);
                    this._context.recordEvent(this.getId(), "IntoBuilding", {"type":type, "time": 6900, "buildingExtInfo":buildingExtInfo}, "CoffeeCatLine");
                }
        ...
        _waitEatTimeOut(info) {
            ...
            this._context.recordEvent(this.getId(), "WaitEatTimeOut", null, "CoffeeCatLine");
        }
        ...
        _outBuilding(info) {//假如猫没了？
            ...
            let buildingId = info[1];
            let buildingExtInfo = info[2];
            buildingExtInfo.setIsBusy(false);
            ...
            if(this._state!="waitEatTimeOut") {
                ...
                // CatLogicHelper.refreshReward({event: eventData, buildingExtInfo: buildingExtInfo, building: building, valueBase: this.catData.getValueBase()})
                let reward = CatLogicHelper.getReward(eventData, this.catData.getValueBase());
                if(reward && reward>0){
                ...
                if(this._state=="eating") ;else if(this._state=="eating") ;
            }
            ...
            this._context.recordEvent(this.getId(), "OutBuilding", {"time": 500, "buildingExtInfo":buildingExtInfo}, "CoffeeCatLine");
    ...
    class CoffeeSceneLogic extends Logic {
        ...
        _attractCat() {
            ...
            let d = Utils.randomExt(catVisitWeightedSum);
            let pre = {w: -1};
            let ret = null;
            for(let i = 0 ; i < catVisitWeightedArray.length; ++i) {
                let cur = catVisitWeightedArray[i];
        ...
        _showCat() {
            if (this._roleMgr.getStackRoleCount() > 0) {
                let buildingExtInfoCount = SceneMgr.getInstance().getScene(Constants.CoffeeSceneId).getBuildingRootInfoCount();
                if(this._roleMgr.getRoleListCount() < buildingExtInfoCount + 12) {
    ...
    class Service {
        ...
        static addCat(catId, handler) {
            // WebSocketClient.getInstance().sendRequest("Cat.AddCatReq", {"catId":catId}, handler);
            Service.sendRequest("Cat.AddCatReq", {"catId":catId}, handler);
        }
        ...
        static updateSceneInfo(customData, rewards, petRewards, visitorRewards, handler) {
            if(petRewards){
                Service.sendRequest("Scene.UpdateSceneInfoReq", {"customData":customData, "rewards":rewards, "petRewards":petRewards, "visitorRewards":visitorRewards}, handler);
            }else{
                Service.sendRequest("Scene.UpdateSceneInfoReq", {"customData":customData, "rewards":rewards, "visitorRewards":visitorRewards}, handler);
            }
        }
    ...
    class System {
        ...
        static _updateBag(bag) {
            if(bag){
                // console.debug("_updateBag arguments", arguments);
                let items = bag.items;
                ...
                Object.getOwnPropertyNames(items).forEach(function(key){
                    ...
                    // console.debug("updateItem", itemId, count);
    ...
    class BeachScene extends Laya.Scene {
        ...
        onAwake() {
            // EventMgr.getInstance().registEvent("ADD_BEACH_CAT", this, this.addCat);
            // EventMgr.getInstance().registEvent("REMOVE_BEACH_CAT", this, this.removeCat);
        ...
        onInitBuilding() {
            ...
            for(var i = 0 ; i < buildingList.length ; ++i) {
                ...
                if(this.buildingMap.get(buildingId) == null && data.getLocationData() != null) {
                    ...
                    let building = BuildingFactory.getInstance().createBuilding(data, this.root, null);
        ...
        showLocationSelect(locationList, buildingId) {
            ...
            locationList.forEach(element=>{
                let selector = BuildingFactory.createBuildingBaseSelector(element, this.chooseLayer, null);
    ...
    class CoffeeScene extends Laya.Scene {
        ...
        onInitBuilding() {
            ...
            for(var i = 0 ; i < buildingList.length ; ++i) {
                ...
                let buildingId = data.getId();
                if(this.buildingMap.get(buildingId) == null && data.getLocationData() != null) {
                    let building = BuildingFactory.getInstance().createBuilding(data, this.root, new Laya.Handler(this, this._updateDropFish));
                    this.buildingMap.set(buildingId, building);
                }
            }
        }
        ...
        onEnable() {
            ...
            this.onInitBuilding();
            // let outSelf = this;
            // let cats = CatSystem.getCatsBySceneId(Constants.CoffeeSceneId);
            // cats.forEach(catData=>{
            //     outSelf.addCat(catData);
            // });
        ...
        _updateDropFish(building) {
            ...
            if(extInfoMgr.getDropFishPoint() == null) ; else {
        ...
        getBuildingRootInfoCount() {
            let cout = 0;
            for (var [key, value] of this.buildingMap) {
                cout += value.getBuildingExtInfoMgr().getBuildingExtInfoCount();
            }
            return cout;
        }
    ...
    class LoginScene extends Laya.Scene {
        ...
        onLogin() {
            Laya.LocalStorage.setItem("user_rid", this.rid);
    ...
    class FishingDialog extends Laya.Dialog {
        ...
        onOpened(data) {
            ...
            let wd = Utils.randomExt(weightedSum);
            let pre = {w: -1};
            let ret = null;
            for(let i = 0 ; i < array.length; ++i) {
                let cur = array[i];
                if(wd > pre.w && wd < (cur.w + pre.w)) {
                    ret = cur;
                    break;
                } else {
                    pre.w += cur.w;
    ...
    */
    ...
    class GameLoader {
        ...
        loadConfig(handler) {
            if(this.configIndex < this.configs.length) {
                let config = this.configs[this.configIndex];
        ...
        start(handler) {
            this.reset();
            this.loadConfig(new Laya.Handler(this, function() {
    ...
    class Main {
    	...
    	onProtoInit() {
    		//测试下notify回调
    		// ProtoManager.getInstance().handlerNotify(1001, "fuck");

    		//测试下Notify的复杂格式proto解析 TODO
    		// var testParam = {"rid" : "fuck", "num" : 123, "testIn" : {"hello" : "zhou", "world": 345}, "testOut": {"fuck": "zz", "you": 789}};
    		// var encode = ProtoManager.getInstance().encodeNotify(1001, testParam);
    		// console.debug("=====");
    		// console.debug(encode);
    		// var decode = ProtoManager.getInstance().decodeNotify(1001, encode);
    		// console.debug("******");
    		...
    		wx.login({
    			success (res) {
    			  if (res.code) {
    				console.log(res);
    				GameLoader.getInstance().start(new Laya.Handler(this, function() {
    					console.debug("显示场景");
