(function () {
    ...
    class BeachCatTLInterpreter extends TLInterpreter{
        ...
        _createCat() {
            if(this._catId) {
                console.debug("BeachCatTLInterpreter._createCat: " + String(this._catId));
                console.debug(CatDataMgr.getInstance().catMap);
                ...
                if(catData) {
                    ...
                    console.debug("Warning BeachCatTLInterpreter._createCat-->未找到对应的猫的数据:" + String(this._catId));
                }
    ...
    class Interpreter {
        ...
        update(frame) {
            let scene1 = SceneMgr.getInstance().getScene(Constants.CoffeeSceneId);
            let scene2 = SceneMgr.getInstance().getScene(Constants.BeachSceneId);
            if(!scene1 || !scene2) {
                return;
            }
        ...
        onShareMenuClick() {
            if(Laya.Browser.onMiniGame) {
                wx.showShareMenu({
                    ...
                    success(ret) {
                        console.debug("调用分享成功!");
                        console.debug(ret);
                    },
                    fail(ret) {
                        console.debug("调用分享失败!");
                        console.debug(ret);
                    }
                });
            } else {
